...
FILENAME_PREFIX_TFEAUTURE = "ftfeatures"
...
class SinteringDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        tfeatures = torch.load(os.path.join(feature_dir,"ftfeatures%d.pt"%index))
        dcon = torch.load(os.path.join(deltas_dir,"fdcon%d.pt"%(index)))
        deta1 = torch.load(os.path.join(deltas_dir,"fdeta1%d.pt"%(index)))
        deta2 = torch.load(os.path.join(deltas_dir,"fdeta2%d.pt"%(index)))
        dtp = torch.load(os.path.join(deltas_dir,"fdtp%d.pt"%(index)))
...
if __name__=='__main__':
    ...
    # tmodel.load_state_dict(torch.load(gt_tmodelname,map_location=device))
    # smodel.load_state_dict(torch.load(gt_smodelname,map_location=device))
    ...
    for istep in tqdm(range(epoch)):
        ...
        for batch in loader:
            ...
            # print("sintering loss",loss_sinter.item(),"tloss",loss_tp.item())
