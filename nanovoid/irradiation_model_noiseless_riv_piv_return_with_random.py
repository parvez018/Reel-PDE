...
class IrradiationSingleTimestep(nn.Module):
        ...
        def forward(self, cv, ci, eta):
                ...
                Pvi,R1,R2 = self.source_term(eta, p_casc, vg)
                Pv = bias * Pvi
                Pi = (1-bias) * Pvi
                Pvi = Pv - Pi
                ...
                # uncomment the following 6 lines to generate noise free simulation data
                ksai1 = 0.
                ksai2 = 0.
                ksai3 = 0.
                # Pi = 0
                ...
                # update
                # ksai need not be learned, a completely random term
                # Pv random matrix, p_casc annd vg MAY need to be learned
                # R_iv r_bulk, r_surf need be learned
                # SvGB, talk with Yexiang, set to constant 0 at the present March 28, 2022
                ...
                return cv_new, ci_new, eta_new, R1,R2
