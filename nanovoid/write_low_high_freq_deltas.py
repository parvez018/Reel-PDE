...
def compute_and_write_all_deltas(datapath,start_id=0,end_id=MAX_FRAME):
    ...
    for index in range(start_id,end_id):
        ...
        all_deltas = torch.load(filepath).to(device)
        ...
        lowfreq_deltas = all_deltas_fft.masked_fill(mask=tmask[None,:,:],value=torch.complex(torch.tensor(0.0),torch.tensor(0.0)))
        
        highfreq_deltas = all_deltas_fft.masked_fill(mask=~tmask[None,:,:],value=torch.complex(torch.tensor(0.0),torch.tensor(0.0)))
        
        lowfreq_deltas_restored = ifft2(lowfreq_deltas).real

        torch.save(lowfreq_deltas_restored,os.path.join(deltas_dir,FILENAME_PREFIX_LOFREQ_DELTAS+suffix))
        torch.save(highfreq_deltas,os.path.join(deltas_dir,FILENAME_PREFIX_HIFREQ_DELTAS+suffix))
...
if __name__=='__main__':
    ...
    print('Feature compute Time: %10f\n'%compute_time)